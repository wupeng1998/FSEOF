...
class FSEOF():
    ...
    def addReactionData(self):
        ...
        for reactionID in list(self.targets.index):
            metabolites.append(self.model.reactions.get_by_id(reactionID).build_reaction_string(use_metabolite_names=False))
            compartments.append(self.model.reactions.get_by_id(reactionID).get_compartments())
            genes.append(self.model.reactions.get_by_id(reactionID).gene_reaction_rule)
            reaction_ID.append(self.model.reactions.get_by_id(reactionID).id)
        
        self.targets["Reaction_ID"] = reaction_ID
        self.targets["Reaction"] = metabolites
        self.targets["Compartments"] = compartments
        self.targets["Genes"] = genes
