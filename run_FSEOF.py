...
def main():
    ...
    if args.useFVA:
        ...
        columns_down = ["Reaction_ID","q_slope_down", "l_sol", "q_slope_classifier", "l_sol_classifier", "reaction_class", "Reaction", "Compartments", "Genes"]
    else:
        columns = ["Reaction_ID","q_slope_up", "Reaction", "Compartments", "Genes"]
        columns_down = ["Reaction_ID","q_slope_down", "Reaction", "Compartments", "Genes"]

    up_targets = up_targets[columns]
    down_targets = down_targets[columns_down]

    # **按 `q_slope_up` 和 `q_slope_down` 的绝对值降序排序**
